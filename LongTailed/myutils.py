...
def saved_path_res(args):
    ...
    if args.dataset == 'ina' or args.dataset == 'imagenet-LT' or args.dataset == 'places-LT' or args.dataset == 'covid-LT' or args.dataset == 'iNaturalist18':
        ...
        if args.imb_factor<1:
            if 'cvardro' in args.loss_type:
                args.res_name = "_".join([
                    args.loss_type +str(args.rho)+ str(args.samplebylabel), "pret"+str(args.pretrained)
                        + "frozen" + str(args.frozen_aside_fc)+ "lbth" , str(args.labelbatch_size), 'sbth', str(args.subbatch_size),'seed', str(args.seed)])
                args.root_log = "PAMI_TrainingResults/" + args.dataset +str(args.imb_type)+str(args.imb_factor) +"/" + args.loss_type+str(args.rho) +str(args.samplebylabel)+ "pret"+str(args.pretrained)\
                    +'resume'+str(args.resume)+"frozen"+str(args.frozen_aside_fc)+"/lbth_"\
                        +str(args.labelbatch_size)+'sbth_'+str(args.subbatch_size)+'/seed_'+str(args.seed)+'/'
            elif 'klrs' in args.loss_type:
                args.res_name = '_'.join([ args.loss_type, \
                'epsilon', str(args.epsilon),'clip', str(args.clip),'clip_th',str(args.clip_threshold), \
                str(args.samplebylabel), "pret"+str(args.pretrained)+'resume'+str(args.resume)
                        + "frozen" + str(args.frozen_aside_fc)+"lbth", str(args.labelbatch_size), 'sbth', str(args.subbatch_size), 'seed', str(args.seed)])
                args.root_log =  "PAMI_TrainingResults/" + args.dataset  +str(args.imb_type)+str(args.imb_factor)+"/"+ args.loss_type+str(args.epsilon) +str(args.samplebylabel)+"pret"+str(args.pretrained)+'resume'+str(args.resume)+"frozen" + str(args.frozen_aside_fc)+"/"\
                        +"lbth_"+ str(args.labelbatch_size)+ 'sbth_'+ str(args.subbatch_size)+'/seed_'+str(args.seed)+'/'
            elif 'abce' in args.loss_type:
                args.res_name = "_".join([
                    args.loss_type + str(args.samplebylabel), "pret"+str(args.pretrained)+'resume'+str(args.resume)
                        + "frozen" + str(args.frozen_aside_fc)+ "lbth" , str(args.labelbatch_size), 'sbth', str(args.subbatch_size),'seed', str(args.seed)])
                args.root_log = "PAMI_TrainingResults/" + args.dataset +str(args.imb_type)+str(args.imb_factor) +"/" + args.loss_type +str(args.samplebylabel)+ "pret"+str(args.pretrained)\
                    +'resume'+str(args.resume)+"frozen" + str(args.frozen_aside_fc)+"/lbth_"\
                        + str(args.labelbatch_size)+ 'sbth_'+ str(args.subbatch_size)+'/seed_'+str(args.seed)+'/'
            else:
                args.res_name = "_".join([
            ...
            if 'cvardro' in args.loss_type:
                args.res_name = '_'.join([ args.loss_type, 'imr', str(args.imb_factor), args.imb_type, \
                'alpha', str(args.rho), "bth", str(args.batch_size), "epochs", str(args.epochs), "arch", str(args.model)])
            elif 'klrs' in args.loss_type:
                args.res_name = '_'.join([ args.loss_type, \
                'target', str(args.target), 'clip', str(args.clip),'clip_th',str(args.clip_threshold), \
                "lbth", str(args.labelbatch_size), 'sbth', str(args.subbatch_size), 'seed', str(args.seed)])
                args.root_log =  "PAMI_TrainingResults/" + args.dataset + "/"+ args.loss_type+str(args.target) +str(args.samplebylabel)+ "/"\
                        +"lbth_"+ str(args.labelbatch_size)+ 'sbth_'+ str(args.subbatch_size)+'/seed_'+str(args.seed)+'/'
            elif 'abce' in args.loss_type:
                args.res_name = "_".join([
                args.loss_type, "imr",
                    "tau", str(args.init_lamda), str(args.lamda),  "bth", str(args.batch_size), "epochs", str(args.epochs), 'lbd_shots', str(args.lamda_shots), 'seed', str(args.seed)])
            elif 'ce' == args.loss_type:
                args.res_name = "_".join([
                    args.loss_type + str(args.samplebylabel), "pret"+str(args.pretrained)
                        + "frozen" + str(args.frozen_aside_fc)+ "lbth" , str(args.labelbatch_size), 'sbth', str(args.subbatch_size),'seed', str(args.seed)])
                args.root_log = "PAMI_TrainingResults/" + args.dataset + "/" + args.loss_type +str(args.samplebylabel)+ "pret"+str(args.pretrained)\
                    + "frozen" + str(args.frozen_aside_fc)+"/lbth_"\
                        + str(args.labelbatch_size)+ 'sbth_'+ str(args.subbatch_size)+'/seed_'+str(args.seed)+'/'
        print(args.root_log)
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.epochs == 30:  # places-LT
        if epoch > 20:
            ...
            lr = args.lr
    elif args.epochs == 90: # imagenet-LT epochLR
        if epoch > 10:
            lr = args.lr * 0.1
        else:
            lr = args.lr
    elif args.epochs == 100:  # imagenet-LT epochLR
...
def get_wieghts_of_each_class(args, loss, targets, u, lamb):

    loss = torch.tensor(loss)
    exploss = torch.exp(loss / lamb)
